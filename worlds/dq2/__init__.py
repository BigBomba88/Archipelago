...
from .Options import DQ2Options
from .Regions import create_regions
from .Rules import set_rules
from .Types import ChapterType, chapter_type_to_name
from .Rom import generate_output, DQ2ProcedurePatch

from .Client import DQ2Client

import settings

class DQ2Settings(settings.Group):
    class DQ2RomFile(settings.UserFilePath):
        """File name of the DQ2 ROM file"""
        description = "Dragon Warrior 2 ROM file"
        copy_to = "dq2.nes"
        md5s = [DQ2ProcedurePatch.hash]

    rom_file: DQ2RomFile = DQ2RomFile(DQ2RomFile.copy_to)
...
class DQ2Web(WebWorld):
    ...
    tutorials = [Tutorial(
        ...
        "A guide to setting up DQ2 for Archipelago. "
...
class DQ2World(World):
    ...
    settings_key = "dq2_options"
    settings: DQ2Settings
    ...
    def fill_slot_data(self) -> Dict[str, object]:
        slot_data: Dict[str, object] = {
            "options": {
                "MysteriousHatFix":         self.options.MysteriousHatFix.value,
                "GlobalEquipment":          self.options.GlobalEquipment.value,
